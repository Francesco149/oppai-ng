...
def gen_test_suite_c():
  with open("test_suite.c", "w") as f:
    wr(f, "/* this code was automatically generated by gentest.py */")
    ...
    for mod in allmods:
      wr(f, '#define %s MODS_%s' % (mod, mod.upper()))
...
def append_test_suite_c(line):
  with open("test_suite.c", "a") as f:
    wr(f, line)
...
for mod in allmods:
  print('#undef %s' % (mod))
  append_test_suite_c('#undef %s' % (mod))
